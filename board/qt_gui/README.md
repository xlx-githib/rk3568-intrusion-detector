...
- [x] **Step 3b 触摸拖动调 ROI**：拖四角=缩放、拖框内=移动；拖动中本地跟手显示，
      **松手才发一条 `roi x y w h`** → 主程序 `Console::submit()` → 与终端命令复用同一套热更新链路
- [x] **Step 3c 参数页 + 按钮**（当前）：右栏双页签（状态 / 参数）；
      参数页 = 停留阈值 / 离开去抖 / conf / nms 的 `−` `+` 按钮 + 上报开关 + ROI 全屏 / 保存配置 / 重新加载

### 页签与按钮（Step 3c）

- 右栏顶部两个页签：**状态**（原来的监看信息 + 事件列表）、**参数**（在线调参）
- 按钮机制很轻：`drawSidebar()` 画按钮时顺手把命中矩形存进 `btns_`，
  `onPress()` 先查命中、`onRelease()` 再执行 —— 不引入任何控件类，也不需要 moc
- **两种命令**：`cmd` 以 `@` 开头 = 本地动作（如 `@page 1` 切页）；否则直接发主程序
  （`stay 5` / `conf 0.30` / `save` …）—— 与终端手敲完全同构
- 按钮布局跟着右栏缓存一起重建（`btns_.clear()`），窗口变尺寸也不会错位
- 参数页的 `−`/`+` 按钮发的是**目标绝对值**（如 `stay %1`），不是增量 ——
  主程序侧无需状态，天然幂等，连点也不会跑飞
